...
def wall_time():return datetime.now(ZoneInfo("Asia/Seoul")).isoformat(timespec="milliseconds")
...
def main():
    ...
    ap.add_argument('--revision',default='s-native-v6-2-7-v77-t1-width')
    ap.add_argument('--lift-mm',type=int,choices=range(12,41),default=20)
    ap.add_argument('--linear',type=int,choices=range(1,1001),default=344)
    ap.add_argument('--duration-ms',type=int,choices=range(500,30001),default=4000)
    ap.add_argument('--fr-extra',action='store_true')
    ap.add_argument('--width-mm',type=int,choices=range(-40,21),default=0)
    ap.add_argument('--legs',choices=['all','rl','rr'],default='all')
    ap.add_argument('--landing-first',action='store_true')
    ap.add_argument('--drive-window',type=float,default=None,help='Wall seconds after started, including internal Stand preparation')
    a=ap.parse_args();a.output.mkdir(parents=True,exist_ok=False)
    if a.drive_window is None:a.drive_window=a.duration_ms/1000+3
    if not 0<a.drive_window<=35:raise ValueError('Drive window must be within 0..35 seconds')
    start=time.monotonic();done=threading.Event();begun=threading.Event();lock=threading.Lock()
    result=dict(started_at=wall_time(),timezone="Asia/Seoul",verification_level="physical_motion",fr_extra=a.fr_extra,width_mm=a.width_mm,lift_mm=a.lift_mm,legs=a.legs,duration_ms=a.duration_ms,command=[a.linear,0],host_drive_window_s=a.drive_window,automatic_retry=False);worker=None;motion=False
    def record(kind,text):
        with lock:
            with (a.output/'events.jsonl').open('a') as f:f.write(json.dumps(dict(timestamp=wall_time(),elapsed_ms=round((time.monotonic()-start)*1000,3),host_s=time.monotonic()-start,kind=kind,text=text))+'\n')
    ...
    with Stm32Console('SpotOMG-Bridge',transport=transport) as c:
        ...
        def heartbeat():
            ...
            while not done.is_set():
                ...
                cmd=f'@D {seq} {a.linear} 0' if elapsed<a.drive_window else f'@S {seq}'
        ...
        try:
            ...
            configured=send(f'probeconfig set {a.lift_mm} {a.linear} {a.duration_ms} {a.legs} {a.width_mm} {int(a.fr_extra)}')
            readback=send('probeconfig show')
            expected={f'lift_mm={a.lift_mm}',f'linear={a.linear}',f'duration_ms={a.duration_ms}',f'legs={a.legs}',f'width_mm={a.width_mm}',f'fr_extra={int(a.fr_extra)}'}
            if not expected.issubset(readback.split()):raise RuntimeError('Probe configuration readback mismatch')
            result['applied_config']=readback
            send('balance status')
            motion=True
            if a.landing_first:
            ...
            motion=True;cmd='walkprobe';record('tx',cmd);transport.write((cmd+'\n').encode())
            ...
            result["ended_at"]=wall_time();result["elapsed_ms"]=round((time.monotonic()-start)*1000,3)
            done.set()
