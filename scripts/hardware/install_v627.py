"""V76 installation stages: measured Landing, torque-off, OTA, readback.
...
OUT=ROOT/'artifacts/s-native-v6-2-7/hardware-v76'
IMAGE=OUT/'firmware/s-native-v6-2-7-v76.bin'
HASH='c312a8ab7afdc2937690b92fb3529e2ed9ef09f0a91007de0ac8d404efdc133c'
...
def main():
    ...
    try:
        with Stm32Console('SpotOMG-Bridge',transport=BleTransport('SpotOMG-Bridge')) as console:
            ...
            required_revision='rev=s-native-v6-2-6-v75' if args.stage=='prepare' else 'rev=s-native-v6-2-7-v76'
            assert required_revision in initial.text.split(),initial.text
            assert {'torque=off','safety=ok','fault_code=0'}.issubset(initial.text.split()),initial.text
            ...
            if args.stage=='prepare':
                ...
                send('gaitprofiles');send('profile');send('syncstate')
