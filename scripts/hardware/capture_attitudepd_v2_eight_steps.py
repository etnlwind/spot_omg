"""One authorized V2 eight-placement trial. No automatic motion retries."""
...
def main():
    ...
    ap.add_argument('--revision',default='attitudepd-v2-v78')
    ap.add_argument('--landing-first',action='store_true')
    ap.add_argument('--drive-window',type=float,default=18.,help='Fallback watchdog window; firmware stops at eight nominal placements')
    a=ap.parse_args();a.output.mkdir(parents=True,exist_ok=False)
    if not 0<a.drive_window<=20:raise ValueError('Drive window must be within 0..20 seconds')
    start=time.monotonic();done=threading.Event();begun=threading.Event();lock=threading.Lock()
    result=dict(requested_steps=8,step_definition="diagonal pair nominal placement; actual ground contact not counted",front_extra_mm=4,legs="all",command=[1000,0],host_drive_window_s=a.drive_window,automatic_retry=False);worker=None;motion=False
    ...
    with Stm32Console('SpotOMG-Bridge',transport=transport) as c:
        def line(s):
            ...
            if s.startswith('$GAITSTEPS '): result['step_report']=s
        ...
        def heartbeat():
            ...
            while not done.is_set():
                ...
                cmd=f'@D {seq} 1000 0' if elapsed<a.drive_window else f'@S {seq}'
        ...
        try:
            ...
            if not all(s in state for s in ('rev='+a.revision,'profile=attitudepd_v2','safety=ok','fault_code=0')):raise RuntimeError('Unexpected preflight state')
            ...
            if max(int(x) for x in re.findall(r'temp=(\d+)C',status))>=70:raise RuntimeError('Servo temperature protection threshold reached')
            send('gaitprofile attitudepd_v2')
            selected=send('syncstate')
            if 'profile=attitudepd_v2' not in selected.split():raise RuntimeError('V2 selection not confirmed')
            send('balance status')
            motion=True
            if a.landing_first:
            ...
            motion=True;cmd='gaitsteps 8';record('tx',cmd);transport.write((cmd+'\n').encode())
            ...
            if 'safety=ok' in result['final_state'].split():
                send('landing',timeout=75)
                landed=send('syncstate')
                if {'pose=landing','torque=on','safety=ok'}.issubset(landed.split()):
                    send('relax');result['final_state']=send('syncstate')
                else:raise RuntimeError('Post-trial Landing not confirmed; no automatic torque-off')
