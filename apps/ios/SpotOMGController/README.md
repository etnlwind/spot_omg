...
- 상태 행에 배터리 전압 추정값 표시: `syncstate` 수신 후 보행 세션이 없을 때
  ...
  조회에 따라 갱신하며, Idle에서는 5초마다 짧은 전압 조회를 추가한다. 보행 중에는
  V71의 전압 알림을 수신한다. 15초가 지난 값은 "이전" 표시, 연결 해제 시 `— V`로
  표시한다. 새 조회 대기 중에도 이전 값과 경고를 유지한다. 11.0V 이하 충전 경고와
  10.5V 이하 즉시 사용 중단 경고를 제공한다. 셀별 전압이나 배터리 단자 직접 측정값이
  아니며 앱 경고와 펌웨어의 보호 정지는 구분한다.
...
### V0.5.0 (29) — Stow 상태의 조작 제한

현재 자세는 제어기의 `$SPOTSTATE pose=...`와 완료 응답으로 표시합니다. Stow/중단 상태에서는 Landing만 자세를 바꿀 수 있습니다. 다른 자세 버튼, 조이스틱, 보행/보정 설정은 비활성화하고 명령 전송 단계에서도 차단합니다. 연결/해제와 상태 조회는 유지합니다. Landing 전환 중 custom 상태가 와도 잠금은 유지하고 `OK landing` 또는 확인된 Landing 상태에서 해제합니다. Stow 완료 즉시 표시를 갱신하며 상태를 재조회합니다.

검증: `tools/check_stow_controls.swift`를 Models/Protocol/BLE Swift 소스와 함께 macOS CLI로 컴파일해 실행. 차단 명령·조이스틱 무전송, Landing 전송, 전환 중 잠금 유지, 완료/재접속 상태 처리를 확인했습니다. iOS 시뮬레이터는 사용하지 않았습니다.

### V0.5.0 (30) — 즉시 조작 잠금과 우선 중지

Stow/Landing/Stand/Stand11 명령을 보내는 즉시 다른 자세·보행·조정 버튼과 조이스틱을 잠급니다. 보행 중 자세를 예약할 때도 바로 잠급니다. 완료 응답(실물의 일반 OK는 후속 자세 조회까지 확인) 또는 거부/중단 응답으로 정리하며, 단순 시간 경과나 오래된 자세 응답으로 풀지 않습니다.

빨간 **Stop** 버튼은 연결된 동안 잠금에 관계없이 사용할 수 있습니다. 자세 전환·수납·보행 중에는 기존 우선 인터럽트 전송 경로로 Ctrl-C를 보내며 예약한 다음 자세도 취소합니다. 가상 로봇의 Stow 중지는 후속 단계와 지연 목표를 비우고 측정된 관절 위치를 유지합니다. 물리 위치/속도를 순간 변경하지 않으며 자동으로 접기·펼치기를 재개하지 않습니다. 실물의 정지 반응은 해당 펌웨어의 기존 인터럽트 처리에 따릅니다.

검증: Swift CLI에서 즉시 잠금·중복 명령 차단·중지 신호 전송·완료/정지 응답 처리를 확인했습니다. 가상 로봇 회귀34개 통과(중간 접기 인터럽트, 위치 순간 변경 없음, 목표 큐 제거, 자동 재개 없음 포함). 실제 끼임 감지나 물리 비상정지 장치의 검증을 의미하지 않습니다.

### V0.5.0 (31) — 오른손 엄지용 Stop 배치

중간 자세 버튼 모음의 Stop을 조종 패널 하단 맨 오른쪽(조이스틱 아래)으로 이동했습니다. 빨간 아이콘·글자·테두리와 옅은 빨간 배경, 76×56pt 터치 영역을 사용합니다. 전환 중 잠금과 무관하게 연결된 동안 활성화되며 기존 우선 중지 신호를 전송합니다. 하단 높이를 고정해 첫 화면 안에 버튼이 들어오도록 했습니다.

### V0.5.0 (32) — Stop 위치 정정

별도의 하단 Stop 버튼을 제거하고 자세 버튼 행의 Relax 바로 오른쪽, 행의 맨 끝에 빨간 Stop을 배치했습니다. 동작 중에도 사용할 수 있는 우선 중지 기능은 유지합니다.

### V0.5.0 (33) — Stop 후 양방향 재개

Stop 완료로 `stow-paused`가 확인되면 Landing과 Stow를 모두 허용합니다. Landing은 멈춘 위치에서 펼치고 Stow는 다시 접으며, 각각 12초의 부드러운 전환을 사용합니다. Stop 응답을 기다리는 동안에는 둘 다 비활성화하고, 재개 명령을 누르면 즉시 다시 잠급니다. 완전히 수납된 Stow 상태에서는 기존처럼 Landing과 Stop만 동작 변경이 가능합니다.

Swift CLI의 정지 응답 전/후 허용 명령 검사와, MuJoCo의 중간 정지 후 Stow 및 Landing 양방향 재개 검사를 통과했습니다. 위치를 순간 변경하지 않고 중단 목표에서 전환을 이어갑니다.

### Stow 접촉 전 구동 종료

가상 로봇의 Stow 목표는 상세 CAD 거리 검사로 줄인 공통 각도를 사용한다.
접기 완료 시 `pose=stow torque=off`로 전환하여 모터가 계속 밀지 않고 중력으로 안착한다.
기존 Stow 버튼이 그대로 적용되며, Landing은 안착한 실제 자세에서 다시 펼친다.
진행 중 Stop과 중지 후 Landing/Stow 재개는 유지한다. 실물 Stow는 아직 지원하지 않는다.

### V0.5.0 (41): 대각선 · 넓은 보폭 80mm

가상 로봇 보행정책에 `대각선 · 넓은 보폭 80mm (검증실패)`를 추가했다.
정책 ID는 `cushion_diagonal_sync_wide80`이며, 2026-09-11 비교 영상 위쪽의
기존 80mm 정책을 그대로 선택한다. 새 MPC/QP 실험 정책과 다르다.
보폭·스윙·몸체 높이·지지 보정 설정은 해당 기존 정책과 동일하다.

빠른 정책 메뉴와 상세 정책 목록에 모두 표시되며 실물 로봇 목록에서는 제외한다.
수평/접촉 동기화 합격 전까지 속도를 합격값으로 표시하지 않는다.
정책 선택·전환·Stop 명령 검사, iPhone 서명 빌드, UJIN17 설치·실행을 확인했다.

### V0.5.0 (42): 원호 턴 · 실물 시험

`arcturn`을 실제·가상 로봇 목록에 추가했다. CAD 원호 발 궤적을 공유 C로 계산하는 별도 실험 정책이며 V38 이상 펌웨어가 필요하다. 기존 Python 전용 80mm 정책 전체를 이식한 것은 아니다. 발 들림·실물 검증이 미완료라 `(검증실패)`로 표시한다. V38 실물 시험은 준비 Stand의 FL-J2 stall로 중단되어 원호 회전 자체는 실행되지 않았다. 상세 기록은 `docs/ARC-TURN-HARDWARE-2026-09-11.md`.

## V0.5.0 (46) 충전 경고 · Mac Catalyst 공통

11.0V 이하에서 큰 빨간 충전 경고, 10.5V 이하에서 즉시 사용 중단 경고를 표시한다.
소리·오류 진동·접근성 알림을 요청한다. 확인은 설명만 접으며 경고는 회복까지 남는다.
Idle에서5초마다 전압을 읽고, V71 전압 스트림은 보행 중에도 수신한다.
기존 펌웨어에서는 보행 종료 후 최소 전압으로도 경고한다.
이 버전부터 프로젝트가 Mac Catalyst를 직접 지원하므로 Mac 임시 복사본 수정은 필요 없다.
