...
static bool sn_stop_place(SNativeControl *s,float dt,GaitPolicyLegTarget out[4]) {
    s->stop_progress=fminf(1,s->stop_progress+dt/1.2f);
    ...
    for(int i=0;i<4;i++) {
        ...
        progress[i]=u;
        /* 0-.12s lift, .12-.24s one placement, .27-.42s lower,
         * then hold until .60s before the other diagonal starts. */
...
bool s_native_step(SNativeControl *s,float phase,float amplitude,float linear,float yaw,
    ...
    for(int i=0;i<4;i++) {
        ...
        float height=swing>0 && swing<1?.012f*activity*sqrtf(fmaxf(0,sinf(GAIT_POLICY_PI*swing))):0;
        /* Right-positive protocol yaw; stance feet oppose body rotation. */
        goal[i][0]=sn_origin[i][0]+amplitude*(.02f*linear*c-.045f*linear*back*back*back+.10f*yaw*sn_origin[i][1]*c+transfer);
