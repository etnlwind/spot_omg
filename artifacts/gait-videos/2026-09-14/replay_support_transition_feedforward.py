...
geometry=configure(r,config);print('GEOMETRY',geometry,flush=True)
out=Path(__file__).with_name('support-transition-feedforward-four-views.mp4')
  ...
  if i%2==0:
   if i<1500:
    ...
    for x,y,label in [(12,0,'SIDE'),(972,0,'TOP'),(12,540,'REAR'),(972,540,'QUARTER')]:
     draw.rectangle((x-4,y+8,x+600,y+65),fill='black');draw.text((x,y+17),f'{label} | {t:05.2f}s | '+('IDLE' if t<10 else 'FORWARD / TRANSFER TRIAL / T3.2s'),fill='white')
    draw.text((650,17),f'MASS 2.754kg | {r.safety}',fill='white')
    draw.text((12,585),f'ROLL {row["roll_deg"]:+.2f}deg | PITCH {row["pitch_deg"]:+.2f}deg | clearance FL/FR/RL/RR mm: '+', '.join(f'{z:.0f}' for z in rows[-1]['foot_clearance_mm']),fill='white')
