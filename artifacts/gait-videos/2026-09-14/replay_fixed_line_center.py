...
out=Path(__file__).with_name('shoulder-j2-aligned-v2-four-views-fixed-line-center.mp4')
...
with mujoco.viewer.launch_passive(m,plant.data) as v:
  ...
  if i%2==0:
   if i<1500:
    ...
    for x,y,label in [(12,0,'SIDE'),(972,0,'TOP'),(12,540,'REAR'),(972,540,'QUARTER')]:
     draw.rectangle((x-4,y+8,x+600,y+65),fill='black');draw.text((x,y+17),f'{label} | {t:05.2f}s | '+('IDLE' if t<10 else 'FORWARD 100%'),fill='white')
 ...
 out.with_suffix('.json').write_text(json.dumps(dict(initial_center_m=initial_center.tolist(),initial_direction=initial_direction.tolist(),geometry=geometry,profile='centerpivot-shoulder-aligned-experiment',idle_s=10,forward_s=20,linear=1,yaw=0,recording_s=30,physics='estimated',rows=rows),indent=2))
