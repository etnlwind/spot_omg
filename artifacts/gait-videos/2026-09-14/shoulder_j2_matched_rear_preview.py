...
def configure(robot):
    ...
    def step(r):
        ...
        # Match diagonal front/rear Cartesian displacement, not servo angles.
        for rear,front in ((2,1),(3,0)):
            targets[rear,[0,2]]=goals[rear,[0,2]]+targets[front,[0,2]]-goals[front,[0,2]]
    ...
    robot.begin=begin
    # Reapply the same X/Z path after legacy balance so it cannot introduce
    # different fore/hind excursions. Uses commanded geometry only, no truth.
    original_apply=robot.balance.apply
    def matched_apply(*args,**kwargs):
        corrected=original_apply(*args,**kwargs)
        if not robot.motion or robot.safety!='ok':return corrected
        kin.set_angles(corrected)
        points=np.array([kin.foot(i) for i in range(4)])
        for rear,front in ((2,1),(3,0)):
            points[rear,[0,2]]=goals[rear,[0,2]]+points[front,[0,2]]-goals[front,[0,2]]
        result,error=kin.solve(points,corrected,iterations=20)
        if error>.001:raise RuntimeError(f'Matched rear path unreachable: {error}')
        kin.set_angles(result)
        reached=np.array([kin.foot(i) for i in range(4)])-goals
        mismatch=max(float(np.max(abs(reached[rear,[0,2]]-reached[front,[0,2]]))) for rear,front in ((2,1),(3,0)))
        info['max_final_front_rear_path_error_mm']=max(info.get('max_final_front_rear_path_error_mm',0),1000*mismatch)
        return result
    robot.balance.apply=matched_apply
    return info
