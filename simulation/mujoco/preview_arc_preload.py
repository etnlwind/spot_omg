...
OUT=ROOT.parents[1]/'artifacts/audits/arc-preload-validation'
def main():
    OUT.mkdir(parents=True,exist_ok=True)
    robots=[];renderers=[]
    for enabled in (False,True):
        p,_=physics();p.update(command_delay_s=.02,tracking_feedback_enabled=False, arc_trial=[.021 if enabled else .02,.5,.04,0], arc_preload_trial=[.3,.04] if enabled else None)
    ...
    path=OUT/'preload-before-after.mp4'
    ...
    try:
        for frame in range(701):
            ...
            for i,(r,renderer) in enumerate(zip(robots,renderers)):
                ...
                draw.text((x+16,8),('기존 원호 턴','하중 보상 · 실험 후보')[i],font=font,fill='white')
                draw.text((x+16,40),f'좌회전 · 명령 지연 20ms · {now:.2f}초'+(' · Stop' if now>=10 else ''),font=small,fill='#b8d5e9')
                ...
                draw.text((x+16,800),f'기울기 {row["roll_deg"]:+.2f}° / {row["pitch_deg"]:+.2f}°',font=small,fill='white')
            ...
            if frame==350:canvas.save(OUT/'preload-before-after-7s.png')
