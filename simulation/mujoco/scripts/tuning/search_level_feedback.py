...
if __name__=='__main__':
    inc=ROOT/'firmware/stm32-learning/Inc';base=(ROOT/'artifacts/simulation/mujoco/diagnostics/imu-balance-v18.h').read_text();folder=Path(tempfile.mkdtemp(prefix='spot-level-feedback-'));paths={}
    ...
    with ProcessPoolExecutor(max_workers=4) as pool:
        for row in pool.map(run,[(g,p,period,height) for period,height in ((.8,.007),(1.,.007)) for g,p in paths.items()]):
            rows.append(row);print(json.dumps(row),flush=True);(RESULTS_ROOT / 'level_feedback_candidates.json').write_text(json.dumps(rows,indent=2)+'\n')
