...
import simulation.mujoco.runtime.cad_physics as cad_physics, simulation.mujoco.scripts.tuning.search_raised_level as search_raised_level
from simulation.mujoco.runtime.virtual_robot import RobotController
ROOT=REPO_ROOT
OUT=ROOT/'artifacts/simulation/mujoco/diagnostics/joint-sport-refined'
...
if __name__=='__main__':
    ...
    specs=[(period,duty,stride) for period in (1.4,1.5) for duty in (.55,.59) for stride in (.10,.105)]
    for n,(period,duty,stride) in enumerate(specs):
        params=[period,duty,stride,.015,.20175,-.035,.75]
