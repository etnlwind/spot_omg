...
import simulation.mujoco.runtime.cad_physics as cad_physics
...
def run(case):
    ...
    row=evaluate((.015,1.4,.065,.64,18.),profile='level15');row.update(j2_lead_s=j2,j3_lead_s=j3)
    return row
...
if __name__=='__main__':
    ...
    for j2,j3 in [(0,.04),(0,.08),(.04,0),(.04,.04),(.04,.08),(.08,.04),(.08,.08),(.06,.10)]:
    ...
    return true;'''.replace('0f*','0.f*')
        d=folder/f'{j2}-{j3}';d.mkdir();(d/'locomotion.h').write_text(base.replace(old,code));lib=d/'policy.dylib'
        ...
        cases.append((j2,j3,str(lib)))
    (ROOT/'artifacts/simulation/mujoco/diagnostics/j3-lift/timing_libraries.json').write_text(json.dumps(cases,indent=2)+'\n')
    rows=[]
    with ProcessPoolExecutor(max_workers=4) as pool:
        for row in pool.map(run,cases):
            rows.append(row);print(json.dumps(row),flush=True)
            (ROOT/'artifacts/simulation/mujoco/diagnostics/j3-lift/timing_search.json').write_text(json.dumps(rows,indent=2)+'\n')
