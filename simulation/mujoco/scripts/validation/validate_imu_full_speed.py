...
from simulation.mujoco.scripts.analysis.diagnose_imu_wobble import build_variants
...
if __name__=='__main__':
    ...
    with ProcessPoolExecutor(max_workers=4) as pool:
        for row in pool.map(run,cases):
            ...
            (RESULTS_ROOT / 'imu_full_speed_validation.json').write_text(json.dumps(rows,indent=2)+'\n')
