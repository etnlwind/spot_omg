...
from simulation.mujoco.scripts.tuning.refine_joint_speed import run
OUT=(RESULTS_ROOT / 'diagnostics')/'joint-speed-refined'
def verify(case):
    candidate,seed=case
...
if __name__=='__main__':
    ...
    with ProcessPoolExecutor(max_workers=4) as pool:
        for row in pool.map(verify,[(candidates[i],seed) for i in (3,7) for seed in (55,77,101)]):
