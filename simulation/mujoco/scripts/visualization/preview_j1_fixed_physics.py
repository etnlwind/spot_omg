...
def main():
    profile=json.loads((ROOT/'config/upright_profiles.json').read_text())['profiles']['cushion_support_shift']
    robots=[];records=[]
    for locked in (False,True):
        height=.24
        for stride in (.14,):
            p,_=physics();p['foot_cushion']=json.loads((ROOT/'config/foot_cushion_d37p3_l27mm.json').read_text())
