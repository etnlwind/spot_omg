...
import numpy as np
import mujoco
from simulation.mujoco.scripts.validation.check_body_height import OUT, ROOT
from simulation.mujoco.scripts.analysis.diagnose_turn_clearance import run
from simulation.mujoco.runtime.cad_physics import foot_clearance
...
def evaluate(case):
    ...
    result,rows=run(1000,0,20.02,profile='sync_'+case,override=cfg,cushion=json.loads((ROOT/'config/foot_cushion_d37p3_l27mm.json').read_text()),observer=observe)
