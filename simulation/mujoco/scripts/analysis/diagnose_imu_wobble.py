...
def build_variants():
    inc=ROOT/'firmware/stm32-learning/Inc';base=(ROOT/'artifacts/simulation/mujoco/diagnostics/imu-balance-v1.h').read_text()
...
if __name__=='__main__':
    ...
    with ProcessPoolExecutor(max_workers=4) as pool:
        for row in pool.map(run,[(n,l,m) for m in ('full','arc','slow') for n,l in paths.items()]):
            ...
            (RESULTS_ROOT / 'imu_wobble_ablation.json').write_text(json.dumps(rows,indent=2)+'\n')
