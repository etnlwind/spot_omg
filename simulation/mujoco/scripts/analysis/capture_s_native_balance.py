...
def main():
    ...
    candidates = [j for j in range(100, 597)
                  if all(r['tilt_deg'] >= 5 for r in records[j:j+4])]
    onset = candidates[0] if candidates else max(range(100, 600), key=lambda j: records[j]['tilt_deg'])
    peak = max(range(onset, min(600, onset+51)), key=lambda j: records[j]['tilt_deg'])
    tipping = next((j for j in range(100, 600) if records[j]['tilt_deg'] >= 45), None)
    toppled = next((j for j in range(100, 600) if records[j]['tilt_deg'] >= 90), None)
    ...
    active = np.array([r['body_relative_x_from_s_mm'] for r in records[200:600]])
    report = dict(profile=args.profile, parameters=PROFILES[args.profile], command=args.command,
        allow_fall=args.allow_fall, wait_s=2, walk_s=10, video_s=12, fps=25,
        ...
        peak_tilt_during_drive_deg=max(r['tilt_deg'] for r in records[100:600]),
    ...
    try:
        ...
        frames = [render(j, 90, 'SIDE') for j in range(max(100,onset-15), min(600,onset+46), 2)]
        ...
        encoders['four-views'] = encoder('four-views', 1280, 840)
        for index in range(0, 600, 2):
            mosaic = Image.new('RGB', (1280, 840), '#17232f')
            draw = ImageDraw.Draw(mosaic)
            draw.text((10, 10), f"{args.profile} | input {args.command/10:.0f}% | 2s wait + 10s walk | same simulation / 4 views", font=font, fill='white')
            for view_index, (name, azimuth, elevation) in enumerate(views):
                ...
                mosaic.paste(frame, ((view_index%2)*640, 40+(view_index//2)*400))
            encoders['four-views'].stdin.write(np.asarray(mosaic).tobytes())
            captures = {min(598,onset+onset%2): 'tilt-onset-four-views.png', 598: 'final-four-views.png'}
            if toppled is not None:
                captures[min(598, toppled+toppled%2)] = 'toppled-four-views.png'
