...
def main():
    profiles=json.loads((ROOT/'config/upright_profiles.json').read_text())['profiles']
    pad=json.loads((ROOT/'config/foot_cushion_d37p3_l27mm.json').read_text())
    results=[]
    for name in ('cushion_support_shift_v1','cushion_support_shift_v2'):
        rows=[]
        def record(now,robot):
            ...
            for i,leg in enumerate(('fl','fr','rl','rr')):
                ...
                rows.append(dict(time_s=now,leg=leg.upper(),phase=q,segment=segment,
                    ...
                    j2_target_deg=float(robot.command_target[3*i+1]),j3_target_deg=float(robot.command_target[3*i+2]),
                    j2_actual_deg=float(np.degrees(d.qpos[plant.q[3*i+1]])),j3_actual_deg=float(np.degrees(d.qpos[plant.q[3*i+2]])),
                    forward_velocity_m_s=float(d.qvel[:3]@forward),
        ...
        run(1000,0,12,profile=name,override=profiles[name],cushion=pad,observer=record)
        with (OUT/f'support-propulsion-{name}.csv').open('w') as stream:
            writer=csv.DictWriter(stream,fieldnames=rows[0]);writer.writeheader();writer.writerows(rows)
        summary={'profile':name,'measurement_seconds':7.,'segments':{}}
    ...
    (OUT/'v2-propulsion.json').write_text(json.dumps(results,indent=2))
