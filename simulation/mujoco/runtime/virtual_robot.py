...
class RobotController:
    ...
    def __init__(self, plant):
        ...
        if self.profile in ("attitudepd", "attitudepd_v2"):
    ...
    def select_profile(self, name):
        ...
        self.profile = name
        if name in ('attitudepd','attitudepd_v2'):
            from simulation.mujoco.runtime.body_stabilizer import BodyStabilizer
    ...
    def command(self, line, now):
        ...
        try:
            if cmd == 'identity':
                ...
                if self.profile in ('attitudepd','attitudepd_v2'):self.body_stabilizer.enabled=words[1]=='on'
        ...
        if kind in ('drive','profile') and self.profile != 'legacy':
            ...
            if self.profile == 'attitudepd_v2':
                ...
                if not fn(NAMES.index(self.profile), phase, amplitude, self.linear, self.yaw, out):
                    raise ValueError('Invalid V2 front clearance target')
    ...
    def begin(self, motion, now):
        ...
        if self.profile in ('attitudepd','attitudepd_v2'):self.body_stabilizer.reset()
    ...
    def rebase_gait_on_s(self):
        if self.profile == 'attitudepd_v2' or self.profiles.get(self.profile,{}).get('s_native'):return
    ...
    def tick(self, now):
        ...
        if self.stow_path:
            ...
            self.balance.applied=False;self.balance.saturated=False
            self.command_target=self.target.copy()
        elif self.profile in ('attitudepd','attitudepd_v2'):
            self.balance.integral[:]=0;self.balance.correction[:]=0
            self.balance.applied=False;self.balance.saturated=False
        ...
        self.flight_frames.append(dict(time_s=float(self.plant.data.time),profile=self.profile,
            ...
            stabilization=self.body_stabilizer.diagnostic.copy() if self.profile in ('attitudepd','attitudepd_v2') else None,
