...
from simulation.mujoco.runtime.cad_gait import CAD, KEYS, build as build_rig, vec
...
class Simulation:
    def __init__(self,p=None,model=None):
        ...
        if self.p.get('aligned_standing_pose',False):
            from simulation.mujoco.runtime.standing_pose import solve_standing_pose
            angles,self.standing_x_error=solve_standing_pose(self.model)
            self.desired=np.radians(angles)
        self.stand_target=np.degrees(self.desired).copy()
    ...
    def step(self,linear=0,yaw=0,balance=True,startup=1,targets_deg=None,limit_yaw=True,stride_scale=None,period_s=None,torque_enabled=True,native_servo=False):
        ...
        if getattr(self,'stow_active',False):
            from simulation.mujoco.runtime.stow_policy import encode
