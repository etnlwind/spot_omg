...
import numpy as np
import pytest
from simulation.mujoco.runtime.dynamics_wbc import constrained_dynamics
...
def test_missing_packet_clears_position_and_load_memory(geometry):
    from types import SimpleNamespace
    from simulation.mujoco.runtime.dynamics_wbc import DynamicsWBC
    g=geometry
...
def test_preview_gravity_contact_schedule_and_pose_bounds():
    from simulation.mujoco.runtime.centroidal_preview import solve_preview
...
def test_imu_velocity_uses_robot_frame_not_rotated_cad_frame(geometry):
    from types import SimpleNamespace
    from simulation.mujoco.runtime.dynamics_wbc import DynamicsWBC
    g=geometry;g.reset()
    params=[1.44,.5,.08,.02,.20175,-.01,.75]
    ...
    nominal=g.plan(params,.2,0.,0.,0.,cfg)
    w=DynamicsWBC(g)
    w.apply(nominal,nominal,SimpleNamespace(filtered=[0,0],previous=[0,0],rate=[0,100]),params,cfg)
    np.testing.assert_allclose(w.data.qvel[3:6],[0,np.radians(10),0],atol=1e-8)
