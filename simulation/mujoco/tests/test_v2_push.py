...
def trial(bias,pulse=0.,peak=.7,shape="sine"):
    ...
    cfg=copy.deepcopy(json.loads((ROOT/'config/upright_profiles.json').read_text())['profiles']['cushion_support_shift_v2'])
    ...
    pad=json.loads((ROOT/'config/foot_cushion_d37p3_l27mm.json').read_text());rec=Recorder();force_rows=[]
    def observe(now,r):
        rec(now,r)
        if now<5:return
