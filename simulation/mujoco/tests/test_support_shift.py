...
from simulation.mujoco.runtime.support_shift import SupportShift, transfer_gate, swing_path
...
def test_final_policy_boundaries_and_stance_constraints(controller):
    profile=json.loads((SIM_ROOT / 'config/upright_profiles.json').read_text())['profiles']['cushion_rear_copy_phase']
...
def test_rear_swing_copies_front_after_attitude_compensation(controller):
    cfg=json.loads((SIM_ROOT / 'config/upright_profiles.json').read_text())['profiles']['cushion_rear_copy_phase']['support_shift']
...
def test_stop_profile_switch_and_imu_loss_preserve_safety(controller):
    ...
    robot.load_experimental_profiles((SIM_ROOT / 'config/upright_profiles.json'))
...
def test_turn_lift_independent_of_turn_speed_and_zero_at_stop():
    from simulation.mujoco.runtime.support_shift import commanded_lift
    cfg={'turn_lift_m':.02}
