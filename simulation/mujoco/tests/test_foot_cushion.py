...
import numpy as np
import pytest
from simulation.mujoco.runtime.cad_physics import build, Simulation, foot_clearance, rounded_cap_vertices
from simulation.mujoco.scripts.tuning.search_gait_profiles import physics

PAD=json.loads((SIM_ROOT / 'config/foot_cushion_d37p3_l27mm.json').read_text())
...
def test_cap_base_parallel_to_cad_triangle_and_length_27mm():
    from simulation.mujoco.runtime.cad_physics import vertices, toe_base_frame
    _,_,m=models()
    for leg in ('fl','fr','rl','rr'):
...
def test_elastic_cap_has_closed_rounded_outline_and_exact_length():
    from simulation.mujoco.runtime.cad_physics import elastic_cap_vertices
