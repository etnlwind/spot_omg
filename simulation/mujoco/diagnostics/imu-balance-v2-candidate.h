...
static inline bool balance_control_apply_internal(BalanceControl *s,GaitPolicyLegTarget target[4],
    ...
    if(enabled) {
        ...
        if(support) {
             ...
             * Do not reverse J1 correction on liftoff. Release it instead. */
            ...
            for(int i=0;i<4;i++) {
                ...
                float dz=preview.down_correction[i]*(.95f+.05f*support[i]);
                float dx=heading_clip(.01f*controlled.pitch,.003f)*placement[i];
                float side=i%2==0?1.f:-1.f;
                corrected[i].j1_deg+=heading_clip(2.f*controlled.roll*side*support[i],1.5f);
