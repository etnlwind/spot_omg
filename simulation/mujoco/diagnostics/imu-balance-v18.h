    ...
    if(enabled) {
        ...
        if(support) {
            ...
            for(int i=0;i<4;i++) {
                ...
                corrected[i].j1_deg+=heading_clip(.5f*input->roll*side*(.9f+.1f*support[i]),1.5f);
...
static inline bool balance_control_apply(BalanceControl *s,GaitPolicyLegTarget target[4],
        const GaitPolicyImuSample *input,bool enabled,bool standing,float kp,float kd,float ki) {
    return balance_control_apply_internal(s,target,input,enabled,standing,kp,kd,ki,NULL);
}
...
static inline bool balance_control_apply_policy(BalanceControl *s,GaitPolicyLegTarget target[4],
    ...
    return balance_control_apply_internal(s,target,input,enabled,standing,kp,kd,ki,support);
}
#endif
