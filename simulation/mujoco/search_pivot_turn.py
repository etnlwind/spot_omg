...
from audit_pivot_center import OUT
ROOT=Path(__file__).resolve().parent

def trial(task):
    name,profile,direction,seconds=task
    p=json.loads((ROOT/'measured_response_plant.json').read_text());p['timestep_s']=.001
    states=[]
    def obs(t,r):
...
def main():
    ...
    base=json.loads((ROOT/'measured_response_profiles.json').read_text())['profiles']['measured_lift']
