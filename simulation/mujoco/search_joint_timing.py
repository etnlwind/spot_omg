...
if __name__=='__main__':
    ...
    (ROOT/'simulation/mujoco/diagnostics/j3-lift/timing_libraries.json').write_text(json.dumps(cases,indent=2)+'\n')
    rows=[]
    with ProcessPoolExecutor(max_workers=4) as pool:
        for row in pool.map(run,cases):
            rows.append(row);print(json.dumps(row),flush=True)
            (ROOT/'simulation/mujoco/diagnostics/j3-lift/timing_search.json').write_text(json.dumps(rows,indent=2)+'\n')
