...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pins : IMU_RST_Pin IMU_CS_Pin */
  ...
  /*
   * Left as a high-impedance input.  PS0 doubles as WAKE and this breakout
   * straps it with a solder jumper to select SPI, so driving the pin would
   * put this output across that strap.
   */
  ...
  /*
   * PA5 is no longer driven here.  It carried the LD2 user LED, but SPI1
   * needs it as SCK, so HAL_SPI_MspInit() claims it as alternate function.
   */
