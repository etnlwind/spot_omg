...
## V13 개선 전진 (`cad-trot5-v13`)

새 STEP/11.1V 3S 물리 모델에서 선택한 C2 발 궤적을 공용 C 정책으로 이식했습니다.
MuJoCo 비교 UI와 STM32가 `gait_policy_trot5_targets()`를 함께 사용합니다.
시뮬레이션 기반 후보이며 실제 로봇의 최적 성능/발열은 아직 검증하지 않았습니다.

```text
trot5            # 기본 3회, 844ms
trot5 3 844      # cycles 1..10, period 844..2400ms
```

- Stand 도착 후 현재 서보 위치에서 1초 동안 개선 보행 준비 자세로 전환합니다.
- 50Hz 목표, 시작/종료 진폭 완화, 기존 혼합 모터 속도·가속도 제한을 사용합니다.
- IMU 자세 보정은 이 보행에만 적용하지 않습니다. IMU 관측·기울기 중단·센서 오류 중단과
  모터 상태 안전 검사는 동작합니다. 기존 보행의 보정 설정을 바꾸지 않습니다.
- 완료 후 현재 위치에서 1초 동안 Stand로 돌아옵니다.
- `syncstate`는 `rev=cad-trot5-v13 caps=trot5`를 전송합니다.
- iOS의 **개선 전진** 버튼 또는 `spotctl trot5`로 호출합니다. 조이스틱의 연속 `drive`
  명령은 기존 전후진/회전 정책을 사용합니다. 새 전진 후보를 혼합 회전에 무검증으로 확장하지 않았습니다.
...
깨끗한 출력 폴더 `/private/tmp/spot-v13-build`에 바이너리·ELF·맵·로그·SHA256 manifest를
만들며 기존 Debug 바이너리를 덮어쓰지 않습니다. 배포 바이너리는
`releases/cad-trot5-v13/cad-trot5-v13.bin`에 보관합니다.
최신 설치/검증 결과는 [인수인계](../../docs/HANDOFF-2026-09-08.md)에 기록합니다.

## 전방 일자 자세 시험 (`forward11-v12`)

이 절은 V12에서 추가한 자세 시험의 기록입니다. V13 업로드 직전 읽기 전용 조회에서
V12 실행 및 torque=off를 확인했습니다. `forward11` 실기 동작은 미실행입니다.

`stand11`은 아래로 곧게 펴는 자세이고, 새 `forward11`은 기본 `stand`에서
네 다리를 앞으로 평행하게 펴는 별도 명령입니다. 기존 `stand`의 보정과
동시 송신 경로는 변경하지 않았습니다.

아래는 명령 형식이며 현재 실행 순서가 아니다. 기본 `stand`는 느린 복귀 명령이
아니므로 전방 명령의 준비 단계로 무조건 실행하지 않는다.

```bash
spotctl --via ble --console-timeout 35 console send forward11
```

`forward11`은 현재 위치가 기본 stand의 관절별 120tick 이내일 때만 시작합니다.
현재 위치를 읽어 hold하고 J1은 유지한 채, 네 J2를 -90도, 네 J3를 0도로
24초 동안 함께 움직입니다. 실기에서 사용자가 v11의 J2 +90도는 뒤쪽을
향한다고 확인하여 v12에서 방향을 수정했습니다. 기존 보정 중심/방향값이나
보행 정책을 바꾸지 않고 이 명령의 목표만 바꿨습니다.
매 20ms 프레임마다 12개 관절의 목표를
하나의 Sync Write로 송신합니다. 관절마다 차례로 이동 완료를 기다리지 않습니다.
위치 보간은 부호 있는 선형 좌표에서 수행하며 360도 wrap이나 기준점 재설정은
하지 않습니다. 버스 지연이 있어도 다음 목표로 급히 따라잡지 않아 실제 시간은
24초보다 길어질 수 있습니다. 최종 도착 확인은 최대 2.5초 추가됩니다.

몸체 roll/pitch가 수평 기준 15도를 넘거나 IMU 읽기가 실패하면 현재 위치를
hold하고 종료합니다. Ctrl+C도 같은 hold 정지입니다. BNO086은 이 명령에서만
500ms보다 오래된 자세 값을 거부하는 reader를 사용합니다. stall·과열·하드웨어
오류는 기존 safety monitor로 검사하며, 서보 통신·위치 범위·도착 시간 오류는
전체 torque off로 종료합니다. hold 정지 자체가 실패해도 torque off를 시도합니다.
이를 실물의 넘어짐 방지 보장으로 간주하면 안 되며, 최초 시험은 몸통을 바로
지지할 수 있는 상태에서 수행합니다.

실제 제어 함수를 모의 서보 버스/시계로 실행하는 시험은 동시 송신, 음의 J2 방향,
프레임별 이동량, 위치 경계, 지연과 timer wrap, 시작 자세 거부, tilt/IMU/interrupt
hold, stall/통신/과열/도착 실패 정지를 검사합니다. 소스 추가 후 CubeIDE의
소스 목록을 재생성할 때 `Src/forward_pose.c`와 기존 `Src/flight_log.c`가
빌드에 포함되어야 합니다. 이번 세션의 개별관절 순차 이동 스크립트는 이 명령의
구현이 아니며 재사용하지 않습니다.

현재 설치·실기 결과는 [실기 작업 기록](../../docs/TRANSPORT-PREFLIGHT-2026-09-07.md)의
마지막 항목을 확인하세요.
