...
kneeflip on|off   앞무릎을 반대로 접기 (같은 발 궤적, 다른 IK 해)
...
이 기체는 앞무릎 서보가 뒷무릎과 반대로 장착돼 있습니다. 정책이 내는 `(J2, J3)`를
그대로 주면 앞다리가 뒷다리와 다르게 움직입니다.

```bash
spotctl console send kneeflip on
```

2링크 평면 다리에는 같은 발 위치에 도달하는 해가 둘 있습니다. 무릎이 반대로
접히는 쪽은 `(J2 - J3, -J3)` 이며, **발끝 좌표는 완전히 동일합니다:**

```text
forward' = sin(J2-J3) + sin(J2-J3+J3) = sin(J2-J3) + sin(J2)   ← 원래와 같음
down'    = cos(J2-J3) + cos(J2)                                 ← 원래와 같음
```

궤적은 그대로 두고 팔꿈치만 반대편으로 보내는 것이라, 앞다리가 뒷다리와 같은
발 경로를 그리면서 무릎만 제 방향으로 접힙니다.
