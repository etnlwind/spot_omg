...
앞다리가 보행 중 뒷다리와 다르게 움직입니다. `stand` 목표 tick을 나란히 놓으면
앞뒤가 짝이 맞지 않습니다.

```text
현재      FL(3) 1036   RL(9) 3020
          FR(6) 3001   RR(12) 1023

앞 J3의 direction만 뒤집으면
          FL(3) 3084   RL(9) 3020     ← 짝이 맞음
          FR(6)  953   RR(12) 1023
```

`center`는 그대로 두고 `direction`만 뒤집으면 앞뒤가 나란해집니다. 다만 이는
**계산이 가리키는 가설**이며, 적용하면 앞무릎이 약 180° 이동하므로 실물에서
확인한 뒤 결정할 일입니다.
