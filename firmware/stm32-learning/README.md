...
`g_robot_gait_forward_signs`가 앞·뒤 거울 대칭 기구에 맞게 적용합니다. 앞뒤가 반대 부호인 것은 의도된 것이며, 넷을 같게 하면 전후 성분의 거울이 상쇄되지 않아 앞발이 stance에서 반대로 밉니다. MuJoCo `walk.py`가 프리셋에서 네 다리를 `-1`로 덮어쓰는 것은 URDF가 무릎 방향을 통일했기 때문이며 시뮬레이터 전용입니다. 허용 범위는
