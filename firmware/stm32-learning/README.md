...
앞다리와 뒷다리는 **기구적으로 동일**합니다. 그런데 앞무릎(J3) 서보 혼이 180°
돌아간 채로 장착돼 있어, 캘리브레이션이 그 반 바퀴를 흡수하고 있습니다.

```text
RL J3 (좌측 뒤):  center 1996, direction +1
FL J3 (좌측 앞):  center   12, direction +1     ← 2060에서 2048(=180°) 뺀 값
FR J3 (우측 앞):  center 4025, direction -1     ← 1977에 2048 더한 값
```

`center`를 2048만큼 옮기고 `direction`을 뒤집는 것이 정확히 반 바퀴를 표현합니다.
**stand 자세는 바뀌지 않습니다** — 캘리브레이션 기준 자세에서는 항등이기 때문입니다
(`ID 3`은 여전히 `1036`, `ID 6`은 `3001`). 바뀌는 것은 그 양쪽, 즉 무릎이 움직이는
방향입니다.

이 값 때문에 앞무릎은 **음의 각도를 표현할 수 없습니다**. 각도 0°가 가동 범위 끝에
놓이기 때문이며, 보행이 쓰는 65~115° 구간은 문제없습니다. `test_spot.py`가 관절별
표현 가능 범위와 보행 구간 도달 가능성을 각각 검사합니다.

혼을 제대로 다시 끼우면 이 오프셋은 불필요해집니다. 그때는 `center`를 원래대로
되돌리고 `direction`을 뒷다리와 같게 맞춥니다.
...
`g_robot_gait_forward_signs`가 다리별로 적용하며 **네 다리 모두 `-1`** 입니다. 이 기체는 앞뒤 무릎이 같은 쪽을 향해 거울 대칭이 아니므로 부호도 같아야 합니다. 옛 `{1,1,-1,-1}`은 자기모순이었습니다 — 같은 쪽 앞뒤 다리가 동일한 각도를 가지면서 한쪽만 지지로 표시됐는데, 각도가 같으면 발 높이도 같으므로 불가능합니다. 대각선 짝이 각도와 지지 위상을 모두 공유하는지는 `test_firmware_c.py`가 검사합니다. 허용 범위는
