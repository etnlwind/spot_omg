...
### 보행 부호 통일: 시뮬레이터와 펌웨어가 서로 다른 보행을 돌고 있었음

거치대 시험에서 뒷다리 자세는 정상인데 앞다리 내딛는 타이밍이 대각선 뒷다리와
어긋난다는 관찰이 나왔습니다.

위상 배정은 원래 맞았습니다. `gait_policy.h`의 `phase_offsets`는
`{0.0, 0.5, 0.5, 0.0}`이라 `FL`↔`RR`, `FR`↔`RL`이 같은 위상입니다. 문제는
`forward_signs`였습니다. **대각선 쌍이 같은 위상이어도 부호가 반대면 같은
순간에 두 발이 반대 방향으로 이동**하므로, 앞다리가 엉뚱한 타이밍에 내딛는
것으로 보입니다.

| | 앞다리 | 뒷다리 |
|---|---|---|
| 펌웨어 `robot_config.c` (기존) | `+1` | `-1` |
| MuJoCo `sim-trot`/`trot2` (검증됨) | `-1` | `-1` |

`walk.py`가 해당 프리셋에서 네 다리를 전부 `-1`로 덮어쓰고 있었고, 10주기
`+2.225m`를 낸 검증 결과가 그 값에서 나온 것이었습니다. 펌웨어만 "앞뒤 기구가
거울 대칭이니 부호가 반대여야 한다"는 옛 가정을 유지하고 있었습니다.

펌웨어를 `{-1, -1, -1, -1}`로 통일했습니다. 뒷다리가 맞다는 관찰을 기준으로
삼았습니다. 전진 방향(네 개 모두 `-1`이냐 `+1`이냐)은 앞뒤 관계와 별개 문제이며
아직 실기에서 확정하지 않았습니다.

`tools/servo_tool/config/joints.json`은 `{FL:-1, FR:-1, RL:+1, RR:+1}`로
`robot_config.c`와 전체 부호가 반전되어 있습니다. 상대 구조는 같으므로 지금
증상과는 무관하지만, 두 파일의 규약이 어긋나 있는 것은 사실입니다. 자동 생성기가
없고 README가 "JSON과 `robot_config.c`를 함께 갱신"이라고만 안내하는 구조라,
이 불일치는 별도로 정리해야 합니다.

### 보행 중 서보 버스 정전

`trot2 10 2000` 1회는 `OK`로 끝났고, 재실행 중 다리가 걸리면서 서보 12축이
전부 `timeout`이 됐습니다.

```text
Starting circular-foot trot2: cycles=10 period=2000ms fold=J2 78/J3 108 balance=on/full
ERROR: servo bus error; servo=1, bus=timeout, servo_error=0x00
$ spotctl scan   -> ID 1..12 전부 timeout
```

**래치된 과부하가 아니라 전원입니다.** STS3215는 과부하로 토크가 꺼져도 ping에는
응답합니다. 12축이 모두 무응답이면 서보 버스 전원이 나간 것이고, 콘솔이 계속
응답한 것으로 보아 STM32와 URT-2는 정상입니다. 다리가 걸리며 전류가 치솟아
전원의 과전류 보호가 걸린 것으로 봅니다.

앞선 무부하 기록에 이미 조짐이 있었습니다. 배리어당 평균 475ms 대기,
`peak_error=314 ticks` — 하중 없이도 서보가 궤적을 못 따라가고 있었습니다.

복구 절차와 재시도 방침은 다음과 같습니다.

1. 서보 전원 재투입 후 `scan`으로 12축 확인
2. 즉시 `relax` — 걸린 자세에 토크가 걸린 채로 두지 않습니다
3. 손으로 다리를 움직여 기구 간섭 지점 확인
4. 주기를 늘려 1주기씩(`trot2 1 3000`) 재시도하고, 3000 → 2500 → 2000으로
   낮춰가며 걸리는 지점을 찾습니다

원인을 찾기 전에는 10주기를 한 번에 돌리지 않습니다. 걸렸을 때 멈출 수단이
`Ctrl+C`뿐입니다.
