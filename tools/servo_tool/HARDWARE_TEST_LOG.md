...
전원, MCU 페리페럴, MCU 핀, 배선 도통, 풀업까지 전부 정상인데 센서 세 개가
모두 침묵합니다. 서로 독립적으로 고장 났다고 보기는 어렵고, 이 저장소에서
측정할 수 있는 범위는 소진했습니다.

다음 확인은 **다른 호스트(Arduino/ESP32)에 센서만 붙여 보는 것**입니다. 거기서
동작하면 이 조립체에 제가 보지 못한 공통 원인이 있는 것이고, 거기서도 안 되면
부품 문제로 확정됩니다.

BNO055 계열 브레이크아웃 중에는 `PS0`/`PS1`을 LOW로 묶어야 I2C가 되는 것이
있습니다. Adafruit 보드는 기판에서 묶여 있지만 클론 보드는 그렇지 않을 수
있으니, 다른 보드를 쓸 때는 이 두 핀의 전압을 먼저 확인합니다.
