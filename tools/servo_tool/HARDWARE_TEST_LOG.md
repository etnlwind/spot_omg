...
- 좌표계, 중립점과 관절 방향은 회사에서 실제 기체를 기준으로 다시 수행한
  calibration (캘리브레이션)을 최종값으로 사용합니다. J1 direction (관절 방향)은
  FL/FR/RL/RR 모두 `+1`입니다.
...
- duty factor (지지율)는 `0.80`, 보폭은 `10°` 상당값, 스윙 높이는 `16°`
  상당값으로 제한했습니다.
- 이는 토크 제한을 올리는 기능이 아니라 기구학적 자세와 접지 시간을 조정하는
  프리셋입니다. 실제 발과 하단 링크를 장착한 뒤에는 `cycles=1`부터 시험해야 합니다.

2026-08-04 고하중 자세와 동시 도착 속도 프로파일 반영 기준 단위 테스트 41개가
통과했습니다.
