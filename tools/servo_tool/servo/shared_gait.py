...
class SharedGaitPolicy:
    ...
    def _build_library(cls) -> Path:
        ...
        digest = hashlib.sha256(
            wrapper.read_bytes() + header.read_bytes() + b"".join(header.with_name(name).read_bytes() for name in ("heading_control.h", "arc_turn.h", "arc_geometry.h", "locomotion.h", "locomotion_profiles.h", "balance_control.h", "drive_control.h", "attitude_control.h", "locomotion_servo.h", "robot_config.h", "motor_capability.h", "stow_control.h", "pose_control.h")) + (header.parent.parent / "Src" / "robot_config.c").read_bytes()
        ).hexdigest()[:16]
