...
**시리얼 터미널과 동시에 쓰지 마세요.** macOS의 `/dev/cu.*`는 여러 프로세스가
동시에 열 수 있고 들어오는 바이트는 먼저 읽는 쪽이 가져갑니다. 터미널 앱을
열어둔 채 `spotctl`을 실행하면 양쪽 다 응답을 반쪽씩 받아 진단이 조용히
어긋납니다. `spotctl`끼리는 배타 잠금으로 막지만, 잠금을 쓰지 않는 터미널
프로그램은 막지 못합니다. 누가 포트를 잡고 있는지는 다음으로 확인합니다.

```bash
lsof /dev/cu.usbmodem312103
```
